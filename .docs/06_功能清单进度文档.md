...
> **测试与验证（2026-07-21）**：已补齐自动化测试套件，覆盖鉴权/限流/解析/分块/审计/定时清理/统一异常/拦截器/对外 API 等核心链路。`mvn test` 全绿：**51 个用例全部通过（0 失败 / 0 错误）**。测试采用 Mockito 单元测试 + standalone MockMvc 控制器测试，对 Redis、百炼 LLM、PostgreSQL 等外部依赖做 mock，可离线运行，无需启动外部基础设施（详见 §8）。
...
- [ ] 全链路审计埋点（登录/权限/KB 增删改/Key 操作/问答）——写入 + 查询/导出已就绪，埋点广度待扩展
...
| 9 | 全链路审计埋点（登录/权限/KB/Key/问答） | 05§11 | P1 | 🟡 部分（写入 + 查询/导出已就绪，埋点广度待扩展） |
...
| 16 | 检索质量评估与调试工具（召回率/命中率） | 02§9、04§6 | P2 | 🔲 缺失 |
| 17 | 数据分级枚举对齐企业标准 | 03§8 | P2 | 🔲 缺失 |
| 18 | 等保留存策略固化（≥6月）、脱敏、密评、内容安全 | 02§9、03§8、05§11 | P1/P2 | 🔲 缺失 |
| 19 | 等保专项测评/整改材料 | 02§9、05§11 | P2 | 🔲 缺失（立项后） |
| 20 | K8s/Helm 部署、TLS、监控（Prometheus） | 05§11 | P2 | 🔲 缺失 |
...
> 统一待定共 22 项：P0 2 项（已完成）、P1 10 项中 8 项已完成 + 1 项部分（#9 审计埋点广度）+ 1 项已拍板（#10）+ 1 项已完成（#22 测试套件）、P2 8 项（产品化/合规/运维，待推进）、决策级 1 项（#21 已拍板对齐）。P1 生产化加固主体已完成（2026-07-21）；剩余以 P2 产品化/合规/运维为主。本文档作为全量待定事项的唯一跟踪入口。

---

## 8 测试与验证（2026-07-21 新增）

> 为落实"测试通过才算任务完成"，补齐自动化测试套件。`mvn test` 全绿：**51 个用例，0 失败 / 0 错误**。
...
| `service/IngestServiceTest` | 单元(mock) | 分块 `splitText`：空白/短文本单块/长文本多块(≤500 硬上限)/保序 | 4 |
...
| `controller/AuditControllerTest` | Web(standalone) | 分页查询返回 content、CSV 导出含表头与数据 | 2 |
...
- 审计埋点广度（§7.3 #9）仍为 🟡 部分：写入 + 查询/导出已就绪，埋点覆盖面待扩展（登录/权限/KB/Key/问答已留痕）。
