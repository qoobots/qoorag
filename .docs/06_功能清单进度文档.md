...
> **测试与验证（2026-07-21）**：已补齐自动化测试套件，覆盖鉴权/限流/解析/分块/审计/定时清理/统一异常/拦截器/对外 API/数据分级校验/检索质量评估等核心链路。`mvn test` 全绿：**77 个用例全部通过（0 失败 / 0 错误）**。测试采用 Mockito 单元测试 + standalone MockMvc 控制器测试，对 Redis、百炼 LLM、PostgreSQL 等外部依赖做 mock，可离线运行，无需启动外部基础设施（详见 §8）。
...
### 2.8 检索质量评估、数据分级与资源池管理（#12 / #16 / #17）
...
> 统一待定共 22 项：P0 2 项（已完成）、P1 10 项全部完成（#9 审计埋点广度已于 2026-07-21 补齐登录/登出/检索/问答埋点）、P2 8 项中 3 项已完成（#12 资源池界面化管理、#16 检索质量评估、#17 数据分级枚举，2026-07-21）、5 项待推进（#13 SSO、#14 连接器、#18 等保留存/脱敏/密评、#19 等保、#20 K8s）、决策级 1 项（#21 已拍板对齐）。P1 生产化加固已全部完成（2026-07-21）；本轮新增 #12/#16/#17 三项 P2 能力；剩余以 P2 产品化/合规/运维为主。本文档作为全量待定事项的唯一跟踪入口。

---

## 8 测试与验证（2026-07-21 新增）

> 为落实"测试通过才算任务完成"，补齐自动化测试套件。`mvn test` 全绿：**77 个用例，0 失败 / 0 错误**。
