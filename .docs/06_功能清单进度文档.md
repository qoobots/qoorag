...
| 对外检索/问答 API（4.10） | 🟡 ~55% | `retrieve` 已真实化（embedding + pgvector 余弦相似度 + 权限）；`chat` 仍为占位 |
| 文档入库流水线 | ✅ ~90% | 上传/解析/分块/向量化链路已实现；支持 TXT + PDF，百炼 text-embedding-v2 |
| 横切基础设施 | 🟡 ~60% | 统一响应、安全上下文、租户隔离已落地；pgvector 原生类型已接入；缺 `@ControllerAdvice`、RLS、限流 |

**总体完成度（功能点加权）：约 73% 骨架完成度；其中核心价值链路（检索/问答/入库）约 55%（入库+检索已实现）。**
...
gantt
    ...
    问答 chat 真实化       :active, m4, after m3, 10d
    section P1 生产化
...
| # | 统一待定事项 | 来源文档 | 优先级 | 状态 |
| --- | --- | --- | --- | --- |
| 1 | RAG 核心链路真实化（retrieve/chat 接入 Embedding+pgvector+LLM） | 05§11、05.3§4.2、05.4§7.2、06§4 | P0 | 🟡 retrieve 已完成，chat 待接入 |
...
> 统一待定共 21 项：P0 2 项（决定产品可用性）、P1 10 项（生产化加固）、P2 8 项（产品化/合规/运维）、决策级 1 项（#21 已拍板对齐）；其中 #10/#15 决策已拍板，仅余实现按对应优先级推进。本文档作为全量待定事项的唯一跟踪入口。
