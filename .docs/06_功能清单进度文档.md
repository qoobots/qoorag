...
| --- | --- | --- | --- | --- |
| 00 头脑风暴 | v0.1 草稿 | 脑暴中 / 待收敛 | 概念级 | 7 项架构决策（02 称已收敛，待与 01 对齐确认） |
| 01 竞品调研 | v0.1 草稿 | 参照分析 | 输入级 | 7 项待定决策待拍板（见 §7.2） |
...
**00 头脑风暴**（状态：待收敛）
- 7 项架构决策已在 02 第 10 章收敛（02 自述"已全部收敛"）；与 01 开头"待逐条拍板"表述需对齐确认。

**01 竞品调研**（§3 对 7 项待定决策的启示）
...
| 5 | 多租户隔离实现（tenant_id 行级隔离 + PostgreSQL RLS） |
| 8 | API 鉴权与限流（OpenAI 兼容 + 按 KB 签 Key + 按 Key 维度限流） |
...
| 15 | 混合检索（向量+BM25）+ Rerank MVP 边界确认 | 02§9、04§6 | P2 | 🔲 待确认 |
...
| 21 | 7 项业务待定决策拍板（01 §3）并与 02 收敛结论对齐 | 01、02 | 决策 | 🟡 02 称已收敛，待对齐 |

> 统一待定共 21 项：P0 2 项（决定产品可用性）、P1 10 项（生产化加固）、P2 8 项（产品化/合规/运维）、决策级 1 项。本文档作为全量待定事项的唯一跟踪入口。
